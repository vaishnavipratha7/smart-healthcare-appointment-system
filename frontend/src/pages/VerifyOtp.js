  ...
  if (!emailFromState) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center py-12 px-4">
        <div className="max-w-md w-full bg-white p-10 rounded-xl shadow-lg text-center space-y-4">
          <h2 className="text-xl font-bold text-gray-900">No email to verify</h2>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-6 bg-white p-10 rounded-xl shadow-lg">
        <div>
          <h2 className="text-center text-3xl font-bold text-gray-900" data-testid="verify-otp-heading">
            Verify your email
          </h2>
          <p className="mt-2 text-center text-sm text-gray-600">
            We sent a 6-digit code to <span className="font-medium text-gray-900">{emailFromState}</span>.
        ...
        {resendMessage && (
          <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-md animate-slide-in" data-testid="resend-message">
            {resendMessage}
          </div>
        )}
        ...
        <form className="space-y-4" onSubmit={handleSubmit} noValidate>
          <div>
            ...
            <input
              ...
              className={`mt-1 block w-full px-3 py-2 border rounded-md shadow-sm text-center text-2xl tracking-[0.5em] focus:outline-none focus:ring-primary-500 focus:border-primary-500 transition-colors ${
                errors.otp ? 'border-red-300 ring-2 ring-red-100' : 'border-gray-300'
              }`}
              placeholder="000000"
              data-testid="otp-input"
            />
            {errors.otp && (
              <p className="mt-1 text-xs text-red-600 font-medium animate-slide-in">{errors.otp}</p>
            )}
          </div>
